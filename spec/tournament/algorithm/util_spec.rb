...
describe Tournament::Algorithm::Util do
  describe '#padd_teams' do
    it 'works for even teams' do
      expect(described_class.padd_teams((1..2).to_a)).to eq((1..2).to_a)
      expect(described_class.padd_teams((1..4).to_a)).to eq((1..4).to_a)
      expect(described_class.padd_teams((1..6).to_a)).to eq((1..6).to_a)
      expect(described_class.padd_teams((1..8).to_a)).to eq((1..8).to_a)
    end

    it 'works for odd teams' do
      expect(described_class.padd_teams((1..3).to_a)).to eq((1..3).to_a + [nil])
      expect(described_class.padd_teams((1..5).to_a)).to eq((1..5).to_a + [nil])
      expect(described_class.padd_teams((1..7).to_a)).to eq((1..7).to_a + [nil])
      expect(described_class.padd_teams((1..9).to_a)).to eq((1..9).to_a + [nil])
    end
  end
  ...
  describe '#all_min_by' do
    it 'finds all minimum elements' do
      ...
      scores = { 1 => 1, 2 => 2, 3 => 3, 4 => 1, 5 => 2, 6 => 3 }
  ...
  describe '#all_perfect_matches' do
    it 'works for 2 elements with size of 2' do
      result = described_class.all_perfect_matches([1, 2], 2).to_a

      expect(result).to eq([[[1, 2]]])
    end

    it 'works for 4 elements with size of 2' do
      result = described_class.all_perfect_matches([1, 2, 3, 4], 2).to_a
    ...
    it 'works for 6 elements with size of 2' do
      result = described_class.all_perfect_matches([1, 2, 3, 4, 5, 6], 2).to_a
    ...
    pending 'works for other sizes' do
      result = described_class.all_perfect_matches([1, 2, 3], 1).to_a
